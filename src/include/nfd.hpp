...
inline void
FreePath(nfdnchar_t* outPath) noexcept
{
...
namespace PathSet {
    inline nfdresult_t
    Count(const nfdpathset_t* pathSet, nfdpathsetsize_t& count) noexcept
    {
        return ::NFD_PathSet_GetCount(pathSet, &count);
    }

    inline nfdresult_t
    GetPath(const nfdpathset_t* pathSet,
    ...
    inline void
    FreePath(nfdnchar_t* filePath) noexcept
    {
        ::NFD_PathSet_FreePathN(filePath);
    }

    inline void
    Free(const nfdpathset_t* pathSet) noexcept
    {
...
/* we need the C++ bindings for the UTF-8 functions as well, because there are
 * different functions for them */

inline void
FreePath(nfdu8char_t* outPath) noexcept
{
...
namespace PathSet {
    ...
    inline void
    FreePath(nfdu8char_t* filePath) noexcept
    {
...
struct PathSetDeleter {
    inline void
    operator()(const nfdpathset_t* ptr) const noexcept
    {
        PathSet::Free(ptr);
    }
};
...
typedef std::unique_ptr<nfdchar_t, PathSetPathDeleter<nfdchar_t>>
        UniquePathSetPath;
typedef std::unique_ptr<nfdnchar_t, PathSetPathDeleter<nfdnchar_t>>
        UniquePathSetPathN;
typedef std::unique_ptr<nfdu8char_t, PathSetPathDeleter<nfdu8char_t>>
        UniquePathSetPathU8;
        ...
        const nfdnchar_t* defaultPath      = nullptr) noexcept
{
    ...
    nfdresult_t res =
            OpenDialogMultiple(out, filterList, filterCount, defaultPath);
    if(res == NFD_OKAY) {
        outPaths.reset(out);
    }
    return res;
        ...
        const nfdnchar_t* defaultName      = nullptr) noexcept
{
    nfdnchar_t* out;
    nfdresult_t res =
            SaveDialog(out, filterList, filterCount, defaultPath, defaultName);
    if(res == NFD_OKAY) {
        outPath.reset(out);
    }
    return res;
...
namespace PathSet {
    inline nfdresult_t
    Count(const UniquePathSet& uniquePathSet, nfdpathsetsize_t& count) noexcept
    {
        return Count(uniquePathSet.get(), count);
    }
    inline nfdresult_t
    GetPath(const UniquePathSet& uniquePathSet,
            nfdpathsetsize_t index,
