...
/* Note: use NFD_PathSet_FreePath to free path from pathset instead of this
 * function */
void
NFD_FreePathN(nfdnchar_t* filePath);

/* initialize NFD - call this for every thread that might use NFD, before
 * calling any other NFD functions on that thread */
nfdresult_t
NFD_Init(void);
...
/* note that some paths might be invalid (NFD_ERROR will be returned by
 * NFD_PathSet_GetPath), so we might not actually have this number of usable
 * paths */
nfdresult_t
NFD_PathSet_GetCount(const nfdpathset_t* pathSet, nfdpathsetsize_t* count);
/* Get the UTF-8 path at offset index */
/* It is the caller's responsibility to free `outPath` via
 * NFD_PathSet_FreePathN() if this function returns NFD_OKAY */
nfdresult_t
...
/* say that the U8 versions of functions are not just #defined to be the native
 * versions */
