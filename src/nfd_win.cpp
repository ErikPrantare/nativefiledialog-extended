...
struct IUnknown;    // Workaround for "combaseapi.h(229): error C2187: syntax
                    // error: 'identifier' was unexpected here" when using
                    // /permissive-
#endif
...
nfdresult_t
AddFiltersToDialog(
        ::IFileDialog* fileOpenDialog,
        const nfdnfilteritem_t* filterList,
        nfdfiltersize_t filterCount)
{
    /* filterCount plus 1 because we hardcode the *.* wildcard after the while
     * loop */
    COMDLG_FILTERSPEC* specList = NFDi_Malloc<COMDLG_FILTERSPEC>(
            sizeof(COMDLG_FILTERSPEC) * (filterCount + 1));
    if(!specList) {
        return NFD_ERROR;
    }
    ...
    if(filterCount) {
        ...
        // use the index that comes from the RAII object (instead of making a
        // copy), so the RAII object will know which memory to free
        ...
        for(; index != filterCount; ++index) {
            ...
            nfdnchar_t* specBuf =
                    NFDi_Malloc<nfdnchar_t>(sizeof(nfdnchar_t) * specSize);
            ...
            *p_specBuf++ = L'\0';
            ...
            // assert that we had allocated exactly the correct amount of
            // memory that we used
        ...
        nfdfiltersize_t filterCount)
{
    ...
    if(*p_spec) {
        ...
        nfdnchar_t* extnBuf =
                NFDi_Malloc<nfdnchar_t>(sizeof(nfdnchar_t) * (numChars + 1));
        if(!extnBuf) {
            return NFD_ERROR;
        }
        ...
        if(!SUCCEEDED(
                   fileOpenDialog->SetDefaultExtension(filterList[0].spec))) {
            return NFD_ERROR;
        }
    }
...
nfdresult_t
SetDefaultPath(IFileDialog* dialog, const nfdnchar_t* defaultPath)
{
    ...
    // SetDefaultFolder() might use another recently used folder if available,
    // so the user doesn't need to keep navigating back to the default folder
    // (recommended by Windows). change to SetFolder() if you always want to
...
nfdresult_t
SetDefaultName(IFileDialog* dialog, const nfdnchar_t* defaultName)
{
...
nfdresult_t
AddOptions(IFileDialog* dialog, FILEOPENDIALOGOPTIONS options)
{
...
void
NFD_FreePathN(nfdnchar_t* filePath)
{
    assert(filePath);
...
nfdresult_t
NFD_PickFolderN(nfdnchar_t** outPath, const nfdnchar_t* defaultPath)
{
    ::IFileOpenDialog* fileOpenDialog;
    ...
    if(!SUCCEEDED(psiResult->GetDisplayName(
...
nfdresult_t
NFD_PathSet_GetCount(const nfdpathset_t* pathSet, nfdpathsetsize_t* count)
{
    assert(pathSet);
    // const_cast because methods on IShellItemArray aren't const, but it
    // should act like const to the caller
    ::IShellItemArray* psiaPathSet = const_cast<::IShellItemArray*>(
            static_cast<const ::IShellItemArray*>(pathSet));
...
void
NFD_PathSet_Free(const nfdpathset_t* pathSet)
{
    assert(pathSet);
    // const_cast because methods on IShellItemArray aren't const, but it
    // should act like const to the caller
...
nfdresult_t
CopyCharToWChar(const nfdu8char_t* inStr, nfdnchar_t*& outStr)
{
...
nfdresult_t
CopyWCharToNFDChar(const nfdnchar_t* inStr, nfdu8char_t*& outStr)
{
    ...
    nfdu8char_t* tmp_outStr =
            NFDi_Malloc<nfdu8char_t>(sizeof(nfdu8char_t) * bytesNeeded);
    if(!tmp_outStr) {
        return NFD_ERROR;
    }
        ...
        FilterItem_Guard& filterItemsNGuard)
{
    if(count) {
        ...
        filterItemsN                    = NFDi_Malloc<nfdnfilteritem_t>(
                sizeof(nfdnfilteritem_t) * count);
        if(!filterItemsN) {
            return NFD_ERROR;
        }
        ...
        for(; index != count; ++index) {
            nfdresult_t res = CopyCharToWChar(
            ...
            res = CopyCharToWChar(
                    filterList[index].spec,
                    const_cast<nfdnchar_t*&>(filterItemsN[index].spec));
            if(!res) {
                // remember to free the name, because we also created it (and
                // it won't be protected by the guard, because we have not
...
nfdresult_t
ConvertU8ToNative(
        const nfdu8char_t* u8Text,
        FreeCheck_Guard<nfdnchar_t>& nativeText)
{
    if(u8Text) {
...
void
NormalizePathSeparator(nfdnchar_t* path)
{
    if(path) {
        for(; *path; ++path) {
            if(*path == L'/')
...
void
NFD_FreePathU8(nfdu8char_t* outPath)
{
    NFDi_Free(outPath);
}
        ...
        const nfdu8char_t* defaultPath)
{
    ...
    nfdnchar_t* outPathN;
    nfdresult_t res = NFD_OpenDialogN(
            &outPathN,
            filterItemsNGuard.data,
            count,
...
/* It is the caller's responsibility to free `outPaths` via NFD_PathSet_Free()
...
/* save dialog */
/* It is the caller's responsibility to free `outPath` via NFD_FreePathU8() if
 ...
 * this function returns NFD_OKAY */
nfdresult_t
NFD_PickFolderU8(nfdu8char_t** outPath, const nfdu8char_t* defaultPath)
{
    // convert and normalize the default path, but only if it is not nullptr
    FreeCheck_Guard<nfdnchar_t> defaultPathNGuard;
    ConvertU8ToNative(defaultPath, defaultPathNGuard);
