        ...
        nfdfiltersize_t filterCount)
{
    ...
    for(nfdfiltersize_t filterIndex = 0; filterIndex != filterCount;
        ++filterIndex) {
        ...
        for(const nfdnchar_t* p_filterSpec = filterSpec; *p_filterSpec;
            ++p_filterSpec) {
            if(*p_filterSpec == ',') {
                // add the extension to the array
                NSString* filterStr = [[[NSString alloc]
                        initWithBytes:(const void*)p_currentFilterBegin
                               length:(sizeof(nfdnchar_t)
                                       * (p_filterSpec - p_currentFilterBegin))
...
static void
SetDefaultPath(NSSavePanel* dialog, const nfdnchar_t* defaultPath)
{
...
static void
SetDefaultName(NSSavePanel* dialog, const nfdnchar_t* defaultName)
{
        ...
        const nfdnchar_t* defaultName)
{
    ...
    @autoreleasepool {
        ...
        // allow other file types, to give the user an escape hatch since you
        // can't select "*.*" on Mac
