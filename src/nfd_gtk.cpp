...
template<typename T>
T*
copy(const T* begin, const T* end, T* out)
{
    for(; begin != end; ++begin) {
        *out++ = *begin;
    }
        ...
        nfdfiltersize_t filterCount)
{
    if(filterCount) {
        ...
        for(nfdfiltersize_t index = 0; index != filterCount; ++index) {
            ...
            *p_nameBuf++                       = '(';
            ...
            for(const nfdnchar_t* p_spec = filterList[index].spec; true;
                ...
                if(*p_spec == ',' || !*p_spec) {
                    ...
                    *p_extnBufEnd++          = '.';
                    p_extnBufEnd =
                            copy(p_extensionStart, p_spec, p_extnBufEnd);
                    *p_extnBufEnd++ = '\0';
            ...
            *p_nameBuf++ = ')';
            *p_nameBuf++ = '\0';
            assert((size_t)(p_nameBuf - nameBuf)
                   == sizeof(nfdnchar_t) * nameSize);
...
Pair_GtkFileFilter_FileExtension*
...
{
    Pair_GtkFileFilter_FileExtension* map =
            NFDi_Malloc<Pair_GtkFileFilter_FileExtension>(
                    sizeof(Pair_GtkFileFilter_FileExtension)
                    * (filterCount + 1));
...
void
SetDefaultPath(GtkFileChooser* chooser, const char* defaultPath)
{
...
void
SetDefaultName(GtkFileChooser* chooser, const char* defaultName)
{
...
static void
FileActivatedSignalHandler(GtkButton* saveButton, void* userdata)
{
            ...
            if(filterMap->filter) {
                ...
                char* appendedFileName = NFDi_Malloc<char>(
                        ...
                        * ((p_period - currentFileName)
                           + (filterMap->extensionEnd
                              - filterMap->extensionBegin)
                           + 2));
                char* p_fileName =
                        copy(currentFileName, p_period, appendedFileName);
                *p_fileName++ = '.';
                p_fileName =
                        copy(filterMap->extensionBegin,
                             filterMap->extensionEnd,
                             p_fileName);
                *p_fileName++ = '\0';
        ...
        const nfdnchar_t* defaultPath)
{
    ...
    if(gtk_dialog_run(GTK_DIALOG(widget)) == GTK_RESPONSE_ACCEPT) {
        // write out the file name
        GSList* fileList =
                gtk_file_chooser_get_filenames(GTK_FILE_CHOOSER(widget));
        ...
        const nfdnchar_t* defaultName)
{
    ...
    buttonClickedArgs.map     = AddFiltersToDialogWithMap(
            GTK_FILE_CHOOSER(widget),
            filterList,
            filterCount);
...
NFD_PathSet_GetCount(const nfdpathset_t* pathSet, nfdpathsetsize_t* count)
{
    ...
    GSList* fileList =
            const_cast<GSList*>(static_cast<const GSList*>(pathSet));
...
void
NFD_PathSet_FreePathN(const nfdnchar_t* filePath)
{
    assert(filePath);
